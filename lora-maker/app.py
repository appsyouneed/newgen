...
def _alias_match(prompt_lower: str, alias: str) -> bool:
    """
    Return True if `alias` appears as a whole-word/phrase match in `prompt_lower`.
    Handles phrases like 'add me', 'the man', 'i', 'me', 'penis', 'dick', etc.
    Uses word-boundary logic so 'me' doesn't match inside 'ame' or 'camera'.
    """
    import re
    # Escape the alias for regex, then wrap in word boundaries
    pattern = r'(?<![a-z0-9_])' + re.escape(alias.lower()) + r'(?![a-z0-9_])'
    return bool(re.search(pattern, prompt_lower))


def match_loras_from_prompt(prompt: str, all_loras: dict) -> dict:
    """
    Scan `prompt` for trigger aliases defined in each LoRA's config and
    return a dict of {lora_id: lora_info} for every LoRA whose aliases fire.

    Also handles the exclusive-identity rule: if the alias signals 'this is me'
    (e.g. 'add me', 'the man', 'dev', 'i', 'me'), the trigger word replaces
    the matched phrase in the prompt AND a 'no other men' negative hint is added
    so the model doesn't invent a second person.

    LoRAs that are already selected by checkbox are NOT re-added here (caller
    merges the two sets).
    """
    prompt_lower = prompt.lower()
    matched = {}
    for lora_id, lora_info in all_loras.items():
        aliases = lora_info.get('config', {}).get('trigger_aliases') or \
                  lora_info.get('trigger_aliases', [])
        if not aliases:
            continue
        for alias in aliases:
            if _alias_match(prompt_lower, alias):
                matched[lora_id] = lora_info
                break
    return matched
...
def apply_lora_prompt_modifications(base_prompt, selected_loras_info):
    ...
    NEW behaviour:
    1. For each active LoRA, check its `trigger_aliases` list against the
       prompt.  If an alias is found, replace it with the canonical trigger
       token (so the model sees the trained word, not 'me' or 'the man').
    2. If the LoRA has identity-exclusivity aliases (those that mean 'only me,
       no other men'), append a negative-style note to prevent a second man
       appearing. This is done by prepending 'only <trigger>, no other men, '
       to the prompt when an identity LoRA is active AND its alias matched.
    3. If no alias matched but the LoRA is checked, append the trigger token
       the old way (mode = append / prepend / replace).
    """
    import re
    ...
    # Identity-signalling aliases — when any of these match, enforce exclusivity
    IDENTITY_ALIASES = {
        "add me", "the man", "dev", "i", "me", "myself", "my body",
        "add her", "the woman", "she", "her", "herself",
    }
    ...
    for lora_info in selected_loras_info.values():
        ...
        # Try to find and replace an alias in the prompt
        alias_replaced = False
        for alias in aliases:
            if _alias_match(prompt_lower, alias):
                # Replace the alias with the canonical trigger token
                pattern = r'(?<![a-z0-9_])' + re.escape(alias.lower()) + r'(?![a-z0-9_])'
                modified_prompt = re.sub(pattern, trigger, modified_prompt, flags=re.IGNORECASE)
                alias_replaced = True

                # Enforce exclusivity for identity aliases
                if alias.lower() in IDENTITY_ALIASES:
                    # Prepend exclusivity guard only if not already present
                    if trigger not in modified_prompt.split(',')[0]:
                        modified_prompt = f"only {trigger}, no other men, no duplicate people, {modified_prompt}"
                break  # one alias match per LoRA is enough

        if not alias_replaced:
            # Fall back to classic append / prepend / replace
            if mode == 'append':
                modified_prompt = f"{modified_prompt}, {trigger}"
            elif mode == 'prepend':
                modified_prompt = f"{trigger}, {modified_prompt}"
            elif mode == 'replace':
                pass  # user-driven via example prompts
...
def animate_frame(
    ...
    # ── ALIAS AUTO-DETECT ────────────────────────────────────────────────────
    # Scan the prompt for trigger aliases defined in loras.json.
    # Any LoRA whose aliases appear in the prompt is activated automatically,
    # even if its checkbox is unchecked. This lets natural phrases like
    # "add me", "the man", "dev", "i", "me", "penis", "dick" fire the right LoRA.
    alias_matched = match_loras_from_prompt(prompt, AVAILABLE_LORAS)
    for lora_id, lora_info in alias_matched.items():
        if lora_id not in selected:
            selected[lora_id] = lora_info
            if selected_loras_info is not None:
                selected_loras_info[lora_id] = lora_info
            print(f"[LoRA] Auto-activated by alias match: {lora_id}")
    # ────────────────────────────────────────────────────────────────────────
    ...
    # Apply trigger-prompt modifications (alias replacement + exclusivity guards).
    if selected:
        if selected_loras_info is None:
            selected_loras_info = {}
        # Merge any alias-auto-detected entries into selected_loras_info
        for k, v in selected.items():
            if k not in selected_loras_info:
                selected_loras_info[k] = v
